...
Then('User should be able to see Master collection landing page with title', async function () {
    await admlandpage.tabMasterCollectionTitleAsBTS.isDisplayed().then(async function () {
        await admlandpage.tabMasterCollectionTitleAsBTS.getText().then(async function (masterTitle) {
            expect(masterTitle).to.equal("BTS");
        })
    })
});
