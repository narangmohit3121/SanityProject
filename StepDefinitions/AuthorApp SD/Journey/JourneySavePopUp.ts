...
import { JourneySavePopUpPageLocate } from "../../../PageObjects/AuthorApp POM/Journey/JourneySavePopUp";
import { browser } from "protractor";
import chai = require('chai');
var expect = chai.expect;
let journeypopuppage = new JourneySavePopUpPageLocate();
let expjourenyttltext: string = 'Journey Description';
let expRegLnkttltext: string = 'registration';
...
When('User refresh the webpage', async function () {
...
When('User clicks on logo', async function () {
    await journeypopuppage.logomomenta.isDisplayed().then(async function () {
        await journeypopuppage.logomomenta.click().then(async function () {
            await browser.sleep(1000);
        })
    })
});

When('User clicks on X icon and try to close the Pop up', async function () {
