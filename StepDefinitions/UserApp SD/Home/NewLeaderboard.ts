...
import { NewLeaderboardPageLocate } from "../../../PageObjects/UserApp POM/Home/NewLeaderboardPage";
...
When("User navigates to Home page", async function () {
    ...
    await browser.wait(EC.invisibilityOf(newLeaderboardPage.shimmerLoadActivity), 5000);
    await newLeaderboardPage.participantNaviationMenu.isPresent().then(async function (navMenuPresent) {
        if (navMenuPresent == true) {
            await newLeaderboardPage.participantNaviationMenu.isDisplayed().then(async function(navMenuDisplayed){
                if(navMenuDisplayed==true){
    ...
    await browser.wait(EC.elementToBeClickable(newLeaderboardPage.optionHomeInMenu), 5000);
...
When("user clicks on Filter by Season dropdown", async function () {
    browser.waitForAngularEnabled(false);
    await browser.wait(EC.visibilityOf(newLeaderboardPage.ddFilterBySeason),5000);
...
Then("the list of sims, regions, Journeys displayed should be correct", async function () {
    ...
    await newLeaderboardPage.ddOptionsIndustry.each(async function (industry) {
        ...
        expect(industryOptions.length).to.be.greaterThan(0);
...
When("User selects the Vertical as {string}", async function (verticalName) {
    let verticalXpath: string = `//select[@name='select vertical']/option[contains(text(),'${verticalName}')]`;

    browser.waitForAngularEnabled(false);
    await browser.wait(EC.elementToBeClickable(newLeaderboardPage.ddFilterByVertical), 5000).then(async function () {
        await newLeaderboardPage.ddFilterByVertical.click().then(async function () {
            let verticalOption: ElementFinder = await element(By.xpath(verticalXpath));
